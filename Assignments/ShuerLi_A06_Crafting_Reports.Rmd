...
```{r data.summary, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
#Compute mean PM2.5 concentration for each county, sorted by value
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(`$\\mu g/m^3$` = mean(PM2.5)) %>% 
  arrange(`$\\mu g/m^3$`)
...
knitr::kable(summary_PM25,caption = "Mean Particulates (2.5mm)",digits = 2)
knitr::kable(summary_Ozone,caption = "Mean Ozone",digits = 2)
...
```{r echo=FALSE, fig.align='left', fig.cap="2018 monthly Ozone levels", warning=FALSE,fig.height=3.5}
epa_data %>%
  filter(year(Date) == 2018) %>%
  ggplot(aes(x = factor(month(Date), labels = month.abb), y = Ozone)) +
  geom_boxplot()+
  labs(
    x = "Month",
...
```
```{r echo=FALSE, fig.align='left', fig.cap="2019 monthly Ozone levels", warning=FALSE, fig.height=3.5}
epa_data %>%
...
https://github.com/ShuerLi387/EDE_Fall2025.git
