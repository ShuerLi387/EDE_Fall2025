...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #其实我没理解这里为什么要c
...
NTL.phys.data. <- filter(NTL.phys.data.Tuesday, year4 %in% c(1990:1999))

#2老师做的
ex2 <- NTL.phys.data %>%
  filter(
    lakename == "Tuesday Lake",
    year4 %in% c(1990:1999) #没有c也可以
  )
...
ex3 <- NTL.phys.data %>% #mutate(month == month(sampledate))
  filter(
    lakename == "Tuesday Lake" &
    year4 %in% 1990:1999 &
    month(sampledate) == 7
